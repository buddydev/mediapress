...
!function() {
/*!********************************************!*\
  !*** ./assets/js/mpp-activity-uploader.js ***!
  \********************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var jquery__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! jquery */ "jquery");
/* harmony import */ var jquery__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(jquery__WEBPACK_IMPORTED_MODULE_0__);
...
(function ($) {
  ...
  $(document).ready(function () {
    let $postSubmitBtn = $('#aw-whats-new-submit'),
        ...
        context = 'activity',
        extensions = getExtensions(uploadSettings.current_type); // Move the buttons if the elemt exists.
    ...
    activityUploader.init(); // When any of the media icons(audio/video etc) is clicked
    // show the dropzone
    ...
    $(document).on('click', '#mpp-activity-upload-buttons a', function () {
      if (activityUploader.isAttached()) {
        ...
        uploadSettings.current_type = $(this).data('media-type'); //use id as type detector , may be photo/audio/video

        setupUploaderFileTypes(activityUploader, uploadSettings.current_type);
        activityUploader.refresh();
        activityUploader.showUI(); // option to disable in 1.4.0
    ...
    $(document).ajaxSend(function (event, jqxhr, settings) {
      let action = (0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.getQueryParameter)('action', settings.data);
      let attached_media = null,
          uploader;
      ...
      switch (action) {
        ...
        case 'new_activity_comment':
          let formID = (0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.getQueryParameter)('form_id', settings.data);
    ...
    $(document).ajaxComplete(function (evt, xhr, options) {
      let action = (0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.getQueryParameter)('action', options.data); //switch
      ...
      switch (action) {
        ...
        case 'new_activity_comment':
          let uploader = getUploader(getUploaderIDForActivity((0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.getQueryParameter)('form_id', options.data)));
    ...
    $(document).on('click', '.mpp-activity-comment-upload-buttons a', function () {
      ...
      mpp_log("Type=" + currentType); // try to find comment ID
      ...
      let uploader = getUploader(uploaderID); // if uploader does exist, we should destroy it if the comment id is different
      ...
      if (!uploader) {
        uploader = new mpp.Uploader(uploaderID, {
          ...
          allowedFileTypes: (0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.prepareExtensions)(getExtensions(currentType)),
          addRemoveLinks: true,
          events: {
            error: function () {//$postSubmitBtn.prop('disabled', false);
            },
            complete: function () {//  $postSubmitBtn.prop('disabled', false);
            },
            allAdded: function () {// $postSubmitBtn.prop('disabled', true);
            }
          }
        });
      ...
      if (uploader.isAttached()) {
        uploader.showUI(); // option to disable in 1.4.0
    ...
    function setupUploaderFileTypes(uploader, type) {
      ...
      uploader.type = type;
      console.log((0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.prepareExtensions)(getExtensions(type)));
      uploader.setAllowedFileTypes((0,_src_utils_functions__WEBPACK_IMPORTED_MODULE_1__.prepareExtensions)(getExtensions(type)));
      /*
...
/******/ })()
;
//# sourceMappingURL=mpp-activity-uploader.dist.js.map